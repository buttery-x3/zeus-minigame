...
Composition is measured at three independent levels: the selected authored family (with procedural selections in their own bucket), the actual feature content of each selected patch, and the final generated microcell structures. Reports are built only from already-committed snapshots; reading a report must not demand cells or generate patches. Failures print aggregate and per-seed counts and percentages, per-seed variance, and the worst complete local windows.
