...
- WFC compatibility uses the edge/socket kinds `open`, `closed`, `river`, and `lake`, then checks the same local grammar constraints so legal small neighborhoods remain the source of truth.
