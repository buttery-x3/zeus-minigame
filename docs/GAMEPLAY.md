...
- Press `F4` or use the pause menu to toggle Terrain Debug, which removes fog, zooms the camera out 3x with debug framing, renders a wider window of already-generated terrain, draws thick amber lines along the true authored/procedural patch boundaries, bypasses visibility-gated movement/cast checks, and keeps Zeus at full HP for terrain inspection.
...
- Terrain cells have a structural type and a derived surface. Patch generation currently emits `open`, `wall`, `lake`, and `river`; `bank` remains reserved for a future post-generation water-adjacency pass. Surfaces include `grass`, `meadow`, `sand`, `mud`, `stone`, `scarred`, `charged`, and `cursed`. Ordinary open ground resolves to green grass or meadow variants; the retired dirt definition and dirt surface are not part of the grammar.
...
- Cliff authored-definition weights are half their previous values. Rivers use topology-group budgets of `5` straight, `15` tight bend, `15` gentle bend, and `2` junction, with junctions authored as tributary confluences rather than downstream splits. Authored river flow is directional: cliff transitions produce one output, continuations connect input to output, and lake mouths consume one input. Opposing authored ports and directed cycles are invalid, although rolling generation may place a lake before constructing its upstream neighbors. River mouths cover the current narrow/broad lake profiles and all approach-angle classes. Isolated all-open lake basins are excluded, cove rotations share one frequency budget, and two cove lake ports cannot connect directly; a flare, shore, core, or mouth must mediate the lake body. Open patch edges carry derived near-edge river, lake, and cliff influence. River/lake and river/cliff features cannot hide within three microcells behind mutually open sockets, preventing long parallel water/ridgeline runs; distance-four arrangements remain legal but disfavored. A compatible transition that joins already committed river and lake context is preferred before normal weights. Procedural repair remains exempt from authored flow semantics and may terminate an arm into its open core when needed to close an otherwise unsatisfied boundary safely. A hard micro-cell topology rule prevents walls, rivers, and lakes—individually or in combination—from enclosing walkable or ungenerated terrain. Among candidates that satisfy that rule, avoidable small river or cliff loops and near-closures remain suppressed as an aesthetic preference.
- Patch edge signatures are ordered lists of `open`, `closed`, `river`, and `lake` sockets. Patch WFC matches each edge against the reversed opposite edge of its neighbor. Authored patches are selected first; a deterministic procedural interior solver supplies a zero-weight termination patch when no topology-safe authored variant fits the accumulated boundary. Procedural blocker arms may remain disconnected around an open core rather than completing a movement barrier.
- The active terrain radius is measured in patch coordinates around the player's current patch. Missing outer-frontier patches are committed in deterministic batches of at most three per frame, direct cell requests remain immediate, and existing patches are never overwritten.
