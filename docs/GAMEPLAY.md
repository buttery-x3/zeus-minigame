...
- Rivers and cliffs use authored endpoints, mirrored sways, asymmetric dog-legs, tight bends, gentle bends, and junctions instead of choosing only straight lines or sharp turns. Lakes retain their existing authored coves, shores, basins, cores, and transitions. Compatible candidates that would create avoidable small river or cliff loops are suppressed; forced connections remain legal so the aesthetic policy cannot break map closure.
