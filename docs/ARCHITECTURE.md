...
- `src/game/perf/Profiler.ts`: rolling frame, subsystem, render, pathfinding, flow-build, navigation-scheduler, and centralized terrain-generation timing metrics. Terrain work is split into budgeted `ensure` and synchronous `demand` sources.
...
`GridWorld` is the timing boundary for all terrain expansion. Explicit rolling work and generation triggered by a cell lookup report `ensure` and `demand` samples to `Profiler`, so terrain work remains attributed even when collision, visibility, or another subsystem requested it. Rolling `TerrainSystem` rendering reads committed cells only and therefore cannot burst-generate its render window. Provider diagnostics retain incremental invariant samples instead of rerunning whole-world enclosure, socket, cove, clearance, and flow audits every time the diagnostics panel refreshes. Diagnostics-panel work itself runs before profiler frame completion under its own subsystem metric.
