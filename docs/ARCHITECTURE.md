...
- `src/world/HexTerrainGrammar.ts`: local committed-pattern terrain grammar and surface derivation.
...
Terrain is generated by a local committed-pattern grammar. Each selected pattern describes one center hex plus its six neighbors. Patterns must agree with already committed terrain and never overwrite it, so nearby generation gradually accretes a coherent local structure instead of rewriting previous cells.
